...
import 'package:mega/features/home/widgets/web/web_store_shimmer_widget.dart';
...
import 'package:mega/common/widgets/item_shimmer.dart';
...
class WebItemsView extends StatefulWidget {
  ...
  const WebItemsView({super.key, required this.stores, required this.items, required this.isStore, this.isScrollable = false,
    this.shimmerLength = 20, this.padding = const EdgeInsets.all(Dimensions.paddingSizeSmall), this.noDataText,
    this.isCampaign = false, this.inStorePage = false, this.isFeatured = false, this.fromStore = false});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      !isNull ? length > 0 ? GridView.builder(
        ...
        padding: widget.padding,
        itemBuilder: (context, index) {
          return ResponsiveHelper.isDesktop(context) ? const WebStoreShimmerWidget()
